...
app.post('/getCommand/:name/:phone', (req, res)=>{
  const {name,phone} = req.params
  const commandArr = req.body.commande
  let command = ''
  commandArr.forEach(c => {
    command+=c
  });
  
  async function getCommandFromClient() {
  ...
  // try{
  //   getCommandFromClient(name,command,phone);
